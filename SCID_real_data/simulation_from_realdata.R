setwd("~/Chen_Zhe")
source("~/Chen_Zhe/reviewpaper_functions.R")

library(RIQITE)
library(dplyr)

################# Methods to be compared ####################
# M1: Adapted from Sedransk and Meyer
# M2: Caughey et al. (2023)
# M3: Combine intervals for treated and control group 
# M4: Berger and Boos's approach
...
compare <- function(dt, k_vec, simul = FALSE){
  ...
  m2_s2 = method_caughey(dt$Z, dt$Y_obs, k_vec = k_vec, method.list = list(name = "Stephenson", s = 2))
  m2_s6 = method_caughey(dt$Z, dt$Y_obs, k_vec = k_vec, method.list = list(name = "Stephenson", s = 6))
  
  # Method M3 is the same for simul = TRUE or FALSE
  m3_treat.s2_control.s6 = method_combine(dt$Z, dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                                           treat.method.list = list(name = "Stephenson", s = 2), 
                                           control.method.list = list(name = "Stephenson", s = 6))
  
  m3_treat.s2_control.s2 = method_combine(dt$Z, dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                                           treat.method.list = list(name = "Stephenson", s = 2), 
                                           control.method.list = list(name = "Stephenson", s = 2))
  
  m3_treat.s6_control.s6 = method_combine(Z= dt$Z, Y = dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                                           treat.method.list = list(name = "Stephenson", s = 6), 
                                           control.method.list = list(name = "Stephenson", s = 6))
  
  m3_treat.s6_control.s2 = method_combine(Z= dt$Z, Y = dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                          treat.method.list = list(name = "Stephenson", s = 6), 
                          control.method.list = list(name = "Stephenson", s = 2))
  
  
  # Method M4 depends on simul
  m4_treat.s2_control.s6 = method_bergerboos(dt$Z, dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                                treat.method.list = list(name = "Stephenson", s = 2), 
                                control.method.list = list(name = "Stephenson", s = 6))
  m4_treat.s2_control.s2 = method_bergerboos(dt$Z, dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                            treat.method.list = list(name = "Stephenson", s = 2), 
                            control.method.list = list(name = "Stephenson", s = 2))
  m4_treat.s6_control.s6 = method_bergerboos(Z= dt$Z, Y = dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                            treat.method.list = list(name = "Stephenson", s = 6), 
                            control.method.list = list(name = "Stephenson", s = 6))
  m4_treat.s6_control.s2 = method_bergerboos(Z= dt$Z, Y = dt$Y_obs, N = n, k_vec = k_vec, simul = simul,
                             treat.method.list = list(name = "Stephenson", s = 6), 
                             control.method.list = list(name = "Stephenson", s = 2))
  ...
  res$method = c(rep('M2-S2', J), rep('M2-S6', J),
                 rep('M3-t.S2-c.S6', J), rep('M3-t.S2-c.S2', J), rep('M3-t.S6-c.S6', J),rep('M3-t.S6-c.S2', J),
                 rep('M4-t.S2-c.S6', J), rep('M4-t.S2-c.S2', J), rep('M4-t.S6-c.S6', J),rep('M4-t.S6-c.S2', J))
...
dir_name = paste0('/home/bzhang3/Chen_Zhe/Results/reviewpaper_simulation_from_realdata_1211')
