...
# Matched dataset
cov_list = c('age', 'sex', 'bmi', 'bpsys', 'bpdia',
             'hematocrit', 'hemoglobin', 'lymphocytes', 
...
### lower limit of prediction intervals for treated units
...
### lower limit of prediction intervals for control units
...
CI_lower.control = CI.control$LB[34:66] 
...
## CIs for all units with different Gamma
...
## proportion of units with positive effects with different Gamma
for (gamma in c(1.2, 1.5, 2.5, 3.3)) {
  ci_lower = readRDS(file = paste0("ci_lower_", gamma,".rds"))
